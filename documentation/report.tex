...
True Prior Nullo (Random Graph) & 0.4025 & 0.5998 & 1.5713 & 0.6402 & 0.0483 \\ \midrule
\textit{Foundation Models (Zero-shot \& Fine-tuned)} \\
TimesFM 2.5 (Zero-shot) & 0.4524 & 0.6960 & 1.9955 & 0.4899 & \textbf{0.0405} \\ 
TimesFM 2.5 (Vanilla Fine-tuned) & 0.6668 & 0.8877 & 1.8950 & 0.2120 & 0.2663 \\
TimesFM 2.5 (SapBERT Fine-tuned) & 0.5022 & 0.7381 & 1.7606 & 0.4553 & 0.2125 \\ \midrule
...
\textbf{Note on SSSD and TimesFM:} The SSSD model training on the 17-feature dataset proved unstable and computationally prohibitive, crashing at epoch 51 with a validation MAE of $\sim$0.66, significantly worse than the linear interpolation baseline (0.40). In contrast, the zero-shot evaluation of \textbf{TimesFM 2.5} demonstrated substantial promise; while its reconstruction metrics are currently behind task-specific models, it achieved the \textbf{lowest correlation error (0.0405)} of all evaluated approaches, highlighting the robustness of foundation model representations for preserving complex medical covariate structures. When fine-tuning TimesFM in a multivariate setting, we observed that pure data-driven fine-tuning struggled (MAE 0.6668). However, when augmented with \textbf{SapBERT embeddings} to guide cross-feature interactions, performance improved significantly (MAE 0.5022), demonstrating the value of external medical knowledge in adapting foundation models.
